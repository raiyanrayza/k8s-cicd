...
add_action( 'pmpro_show_user_profile', 'extra_fields_for_edit_profile' );
function extra_fields_for_edit_profile(){
	$image_icon = '';
	$contact_card_icon = '';
	...
	if( !empty( $user_ep_id ) ){
		...
		if( $photo ){
			...
			$image_icon = '<img class="user-icn" src="'. esc_url($thumb) .'" alt="icon" width="100"/>';
		}*/
		...
		if($thumb)
			$image_icon = '<img class="user-icn" src="'. esc_url($thumb[0]) .'" alt="icon" width="100"/>';
	}
	?>
	<div class="pmpro_member_profile_edit-fields paces_custom-fields">
		<!-- <div class="pmpro_member_profile_edit-field pmpro_member_profile_edit-field- upload_photo">
			<div class="fields-wrap">
				<label for="upload_photo">upload photo</label>
				<input type="file" name="upload_photo" id="upload_photo" class="input">
			</div>
		...
		if( !empty( $user_ep_id ) ){
			/*
			$image = get_field( 'contact_card_vcard', $user_ep_id );
			if( $image['type'] == 'image' ){
				$size = 'thumbnail';
				$thumb = $image['sizes'][ $size ];
				$contact_card_icon = '<img class="contact-icn" src="'. esc_url($thumb) .'" alt="icon" width="100"/>';
			}
			?>
			<div class="pmpro_member_profile_edit-field pmpro_member_profile_edit-field- upload_contact">
				<div class="fields-wrap">
					<label for="upload_contact">Contact card (vCard) <small>only for EP Members</small></label>
					<input type="file" name="upload_contact" id="upload_contact" class="input">
				</div>
				<div class="preview-wrap">
					<?php echo $contact_card_icon; ?>
				</div>
			</div>
			<?php
			*/
...
// add_action( 'paces_pmpro_show_user_profile_above', 'extra_fields_for_edit_profile_above',10,1 );
add_action( 'paces_pmpro_show_user_profile_above', 'extra_fields_for_edit_profile_above',10,1 );
function extra_fields_for_edit_profile_above($user){
	...
	<input type="hidden" name="user_email" id="user_email" value="<?php echo esc_attr( $user->user_email ); ?>" />
	<script>
		( function($){
