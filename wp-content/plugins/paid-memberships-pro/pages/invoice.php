...
<?php
	if($pmpro_invoice)
	{
		...
		<?php
			...
			$approved_check = intval( get_user_meta($current_user->ID,'pay_approved_by_admin',true) );
		?>
		<h5><?php printf(__('Invoice #%s on %s', 'paid-memberships-pro' ), $pmpro_invoice->code, date_i18n(get_option('date_format'), $pmpro_invoice->timestamp));?></h5>
		...
		<ul>
			...
			<li><strong><?php _e('Status', 'paid-memberships-pro' ); ?>:</strong>
				<?php echo ! empty( $pmpro_invoice->status ) ? ucwords( $pmpro_invoice->status ) : __( 'Success', 'paid-memberships-pro' );
				
				if( $refunded_data ){
					if( isset( $refunded_data[$pmpro_invoice->id] ) ){
						if( $refunded_data[$pmpro_invoice->id]['status']=='succeeded' ){
