...
function pmpro_shortcode_account($atts, $content=null, $code="")
{
	...
	/* added by bitcot */
	$refunded_data = get_user_meta( $current_user->ID, 'refund_data', true );
	if( $refunded_data ){
		$refunded_data = maybe_unserialize( $refunded_data );
	}
	/* added by bitcot 26-10-2020*/
		...
		<?php if(in_array('profile', $sections)) { ?>
			<div id="pmpro_account-profile" class="<?php echo pmpro_get_element_class( 'pmpro_box', 'pmpro_account-profile' ); ?>">
				...
				<?php if($current_user->user_firstname) { ?>
					<p class="my_account_name-label"><?php echo $current_user->user_firstname?> <?php echo $current_user->user_lastname?></p>
				<?php } ?>
				<ul>
						...
						<tr>
							...
							<?php
								foreach($mylevels as $level) {
									if($level->id != 1){
							?>
							<tr class="pmpro_level_id_<?php echo $level->id; ?>">
							...
							<?php }else if( !get_user_meta( $current_user->ID, 'first_payment', true ) ){
								?>
								<tr>
								<td colspan="3">
								<?php
								...
								<?php
							}else if( get_user_meta( $current_user->ID, 'first_payment', true ) == 'true' ){
								?>
								<tr>
								<td colspan="3">
								<?php
								$url = pmpro_url( 'levels' );
								printf( __( "Your <strong>membership request is in pending</strong> you will get notify once its approved by admin", 'paid-memberships-pro' ), $url );
								?>
									</td>
								</tr>
								<?php
			...
			<table class="<?php echo pmpro_get_element_class( 'pmpro_table' ); ?>" width="100%" cellpadding="0" cellspacing="0" border="0">
				...
				<?php
					...
					foreach($invoices as $invoice)
					{
						...
						if( $refunded_data ){
							if( isset( $refunded_data[$invoice_id] ) ){
								if( $refunded_data[$invoice_id]['status']=='succeeded' ){
									$display_status .= ' (Refunded)';
								}
							}	
						}
						/*$display_status .= '-'.$invoice_id;*/
						?>
