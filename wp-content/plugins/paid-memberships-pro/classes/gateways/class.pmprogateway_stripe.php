...
class PMProGateway_stripe extends PMProGateway {
	...
	/**
	 * get_payment_method_id_by_custom_id cusotm added by bitcot on 28-sep-2020 .
	 *
	 */

	function get_payment_method_id_by_custom_id($cid){
		// Stripe_Customer::retrieve( $customer_id )
		try {
			$alls_cards = Stripe_Customer::retrieve(
				$cid,[]
			);
			if($alls_cards){
				if(isset($alls_cards['invoice_settings']['default_payment_method'])){
					$payment_method_id_x = $alls_cards['invoice_settings']['default_payment_method'];
					return $payment_method_id_x;
				}
			}
		} catch (\Throwable $th) {
			return false;
	...
	/**
	 * paces_update_card_by_token cusotm added by bitcot on 13-oct-2020 .
	 *
	 */

	function paces_update_card_by_token($post){
		$stripee = new \Stripe\StripeClient(pmpro_getOption( "stripe_secretkey" ));

		$user_id = isset($post['user_id']) ? $post['user_id'] : '';
		$token = isset($post['token']) ? $post['token'] : '';
		
		if( get_user_meta( $user_id, 'pmpro_stripe_customerid', true ) ){
			$stripe_cid = get_user_meta( $user_id, 'pmpro_stripe_customerid', true );
			// $stripe_cid = 'cus_I7HZF3OOctaJn5';

			try {
				$PMResponse = Stripe_PaymentMethod::create([
				  'type' => 'card',
				  'card' => [
				    'token' => $token,
				  ],
				...
				if($PMResponse){
					if($PMResponse['id']){
						$pm_id = $PMResponse['id'];
						try {
							$pm_attached = $stripee->paymentMethods->attach(
										  $pm_id,
										  ['customer' => $stripe_cid]
										);
							...
							if($pm_attached){
								try{
									$upateCust = $stripee->customers->update(
										  $stripe_cid,
										  ['invoice_settings' => ['default_payment_method' => $pm_attached['id']]]
										);
									if($upateCust){
										return array( 'type'=>'success', 'msg'=>$upateCust );
									}
								}  catch ( \Throwable $th ) {
									return array( 'type'=>'error_default_set', 'msg' => $th->getMessage() );
								}
							}
						} catch ( \Throwable $th ) {
							return array( 'type'=>'error_attach', 'msg' => $th->getMessage() );
						}
					}
						
				}	
			} catch (\Throwable $th) {
				return array( 'type'=>'fail', 'msg' => $th->getMessage() );
			}
	...
	function create_payment_intent( &$order ) {
		...
		$amount = pmpro_round_price( (float) $order->subtotal + (float) $tax );
		//https://stackoverflow.com/questions/29753923/stripe-update-customer-default-card-php
		$params = array(
			'customer'            => $this->customer->id,
	...
	function confirm_payment_intent( &$order ) {
		...
		try {
			...
			$res = $this->payment_intent->confirm( $params );

			if( isset($order->done_by) ){
				$pay_m = $res->charges->data[0]->payment_method_details;
				$st = $res->status;
				if($st = 'succeeded'){
					$st = 'success';
				}
				$order->payment_transaction_id = $res->charges->data[0]->id;
				$order->cardtype = $pay_m->card->brand;
				$order->accountnumber = $pay_m->card->last4;
				$order->expirationmonth = $pay_m->card->exp_month;
				$order->expirationyear = $pay_m->card->exp_year;
				$order->status = $st;
			}
		} catch ( Stripe\Error\Base $e ) {
			$order->error = $e->getMessage();
